  ...
  return (
    <div>
        ...
        <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>
          <button className={`y-1 px-3 border rounded text-sm transition-all sm:hidden ${showFilter?'bg-indigo-500 text-white' :''}`} onClick={()=>setShowFilter(prev=>!prev)}>Filters</button>
        ...
        <div className='w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-6 mt-6'>
        {
          filterDoc.map((item, index) => (
          <div onClick={()=>navigate(`/appointment/${item._id}`)} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer w-full max-w-[220px] mx-auto transition-all duration-300 transform hover:-translate-y-2 hover:shadow-lg'
            key={index}
          >
